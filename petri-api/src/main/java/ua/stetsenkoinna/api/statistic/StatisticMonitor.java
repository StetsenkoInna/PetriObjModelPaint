...
public class StatisticMonitor {
    ...
    public StatisticMonitor() {
        ...
        this.lastStatisticCollectionTime = dataCollectionConfig.getDataCollectionStartTime()
                - dataCollectionConfig.getDataCollectionStep();
    }
    ...
    public Map<Integer, List<String>> getWatchMap() { return watchMap; }

    public boolean isValidWatchList() {
        return watchMap != null && !watchMap.isEmpty();
    }

    public Double getDataCollectionStartTime() { return dataCollectionConfig.getDataCollectionStartTime(); }
    public Double getDataCollectionStep() { return dataCollectionConfig.getDataCollectionStep(); }
    public Double getLastStatisticCollectionTime() { return lastStatisticCollectionTime; }
    ...
    public List<PetriElementStatisticDto> getNetWatchListStatistic(Integer petriObjId, PetriNet petriNet) {
        ...
        petriStat.addAll(Arrays.stream(petriNet.getListP())
                ...
                .map(petriP -> new PetriElementStatisticDto(petriObjId, petriP.getName(),
                        petriP.getObservedMin(), petriP.getObservedMax(), petriP.getMean()))
                .toList());
        petriStat.addAll(Arrays.stream(petriNet.getListT())
                .filter(petriT -> watchList.contains(petriT.getName()))
                .map(petriT -> new PetriElementStatisticDto(petriObjId, petriT.getName(),
                        petriT.getObservedMin(), petriT.getObservedMax(), petriT.getMean()))
                .toList());
        return petriStat;
    }
}
