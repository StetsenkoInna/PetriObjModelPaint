...
public class PetriObjModel implements Serializable, Cloneable  {
     ...
     * Simulation protokol is printed on console.
     *
     ...
     * @param area specifies where simulation protokol is printed
     */
 /*   public void go(double timeModeling, JTextArea area) { //виведення протоколу подій та результатів моделювання в об"єкт класу JTextArea
        if(area==null){
            this.go(timeModeling);
            return;
        }
        area.setText(" Events protocol ");
        this.setSimulationTime(timeModeling);   //3.12.2015
        this.setCurrentTime(0.0); 
        double min;
        getListObj().sort(PetriSim.getComparatorByPriority());
        for (PetriSim e : getListObj()) {
        ...
        while (this.getCurrentTime() < timeModeling) {
                ...
                for (PetriSim e : getListObj()) {
                    if (min > 0) {
                        if(min<timeModeling)
                            ...
                            e.doStatistics((timeModeling - this.getCurrentTime()) / timeModeling); 
                    }
                }
            }
            ...
            this.printInfo(" \n Time progress: time = " + this.getCurrentTime()  + "\n",area);
         
            if (this.getCurrentTime()  <= timeModeling) {

                for (PetriSim e : getListObj()) {
                    if (this.getCurrentTime() == e.getTimeMin()) { // розв'язання конфлікту об'єктів рівноймовірнісним способом
                        conflictObj.add(e);
                    }
                }
                int num;
                int max;
                this.printInfo("  List of conflicting objects  " + "\n", area);
                for (int ii = 0; ii < conflictObj.size(); ii++) {
                    this.printInfo("  K [ " + ii + "  ] = " + conflictObj.get(ii).getName() + "\n", area);
                }
                ...
                for (PetriSim sim : getListObj()) {
                    if (sim.getNumObj() == conflictObj.get(num).getNumObj()) {
                        this.printInfo(" time =   " + this.getCurrentTime() + "   Event '" + sim.getEventMin().getName() + "'\n" 
                                + "                       is occuring for the object   " + sim.getName() + "\n", area);
                        sim.doT();
                        sim.output(); //17.07.2018
                    }
                }
                this.printInfo("Markers output:", area);
                this.printMark(area);
                ...
                this.printInfo("Markers input:",area);
                this.printMark(area);
            }
        }
        this.printInfo("\n Simulation results: \n", area);
        ...
        for (PetriSim e : getListObj()) {
           this.printInfo("\n Petri-object " + e.getName(), area);
           this.printInfo("\n Mean values of the quantity of markers in places : ", area);
            for (PetriP P : e.getListPositionsForStatistica()) {
                this.printInfo("\n  Place '" + P.getName() + "'  " + Double.toString(P.getMean()), area);
            }
            this.printInfo("\n Mean values of the quantity of active transition channels : ", area);
            for (PetriT T : e.getNet().getListT()) {
                this.printInfo("\n Transition '" + T.getName() + "'  " + Double.toString(T.getMean()), area);
            }
        }
    }*/
